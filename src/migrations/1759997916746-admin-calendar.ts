...
export class AdminCalendar1759997916746 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE OR REPLACE FUNCTION get_provider_shifts_for_month_admin (
	provider_uuid UUID,
	start_date DATE,
	end_date DATE
) RETURNS TABLE ("date" CHARACTER VARYING, shifts JSON) AS $$
        BEGIN
            RETURN QUERY
            WITH date_series AS (
                SELECT
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP FUNCTION IF EXISTS get_provider_shifts_for_month_admin(UUID, DATE, DATE)`,
    );
  }
}
